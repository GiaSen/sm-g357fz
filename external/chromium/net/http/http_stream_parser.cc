...
//> RNTFIX
  ...
  if(request->valid_uid)
  {
    callUid = (unsigned int)request->calling_uid;
...
void HttpStreamParser :: getUidInfo(unsigned int *callUid, unsigned int *uid)
{
  *callUid = this->callUid;
  *uid = this->uid;
  if(this->callUid)
  {
     if(printLog)LOG(ERROR) << "Valid UID present "<<this->callUid <<" Uid "<<this->uid;
  }
}
...
bool HttpStreamParser::isNetworkServerAddress()
{
  ...
  else if(servAddr->ai_addr->sa_family ==  AF_INET6)
  {
    ...
    if(inet_ntop(AF_INET6, &(((sockaddr_in6*)(servAddr->ai_addr))->sin6_addr), ipstr6, INET6_ADDRSTRLEN) == NULL)
    {
      if(printLog)LOG(ERROR) << " invalid ipv6 address " << ipstr6;
      return false;
    }
    
    ipv6 = ((sockaddr_in6*)(servAddr->ai_addr))->sin6_addr.s6_addr;
    
    if(0 == strcmp(ipstr6,"0:0:0:0:0:0:0:1"))
    {
       isloopback = true;
    }
    if(ipv6[0] == 0xff)
    ...
    if(ipv6[0] == 0xfe && ipv6[1] == 0xc0)
    {
      issitelocal = true;
    }
    if(ipv6[0] == 0xfe && ipv6[1] == 0x80)
    {
      islinklocal = true;
    }
  ...
  {
    if(printLog)LOG(ERROR) << " Unknown address ";
    return false;	
  }
}
